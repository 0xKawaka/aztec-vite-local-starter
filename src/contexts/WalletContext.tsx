...
export const WalletProvider = ({ children, pxeUrl }: WalletProviderProps) => {
  ...
  useEffect(() => {
    const initializeWallet = async () => {
      try {
        ...
        setError(error instanceof Error ? error : new Error('Failed to initialize wallet'));
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <WalletContext.Provider value={{ wallet, address, availableWallets, switchWallet, isLoading, error }}>
      {children}
